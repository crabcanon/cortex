...
class ParsePersistenceService:
    ...
    async def persist(
        ...
        *,
        ...
        if attempts_to_add:
            uow.parse_run_attempts.add_many(attempts_to_add)
    ...
    async def _persist_document_graph(
        ...
        *,
        ...
        if tags_to_add:
            uow.document_tags.add_many(tags_to_add)
        ...
        if chunks_to_add:
            uow.document_chunks.add_many(chunks_to_add)
        ...
        if request.persistence.persist_artifacts and result.artifacts is not None:
            ...
            if artifacts_to_add:
                uow.document_artifacts.add_many(artifacts_to_add)
