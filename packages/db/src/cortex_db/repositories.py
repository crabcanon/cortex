...
class JobEventRepository:
    ...
    async def get_max_sequence_no(self, job_id: str) -> int:
        # Optimized O(1) query to fetch the max sequence number directly on the DB
        # This avoids loading thousands of records into memory just to check the last index.
        result = await self._session.execute(
