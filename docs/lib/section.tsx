...
export function createSection(config: SectionConfig) {
  ...
  async function Layout({ children, params }: SectionLayoutProps) {
    const locale = getPathLocale(params ? await params : undefined);
    const { nav, ...rest } = baseOptions();
    const changelogHref =
      locale === "zh" ? "/zh/changelog/cortex-2026" : "/changelog/cortex-2026";
    ...
    return (
      <>
        <Banner id="docs-announcement" height="30px">
          {locale === "zh" ? (
            <>
              Cortex 文档已覆盖 Parse、Storage、Knowledge、Evaluation 与
              Synthesis。查看 <Link href={changelogHref}>最新变更</Link>。
            </>
          ) : (
            <>
              Cortex docs now cover Parse, Storage, Knowledge, Evaluation, and
              Synthesis. See the <Link href={changelogHref}>latest changelog</Link>.
            </>
          )}
        </Banner>
        <DocsLayout
  ...
  async function Page(props: SectionPageProps) {
    ...
    const locale = getPathLocale(params);
    const rawPage = source.getPage(params.slug, locale);
    if (!rawPage) notFound();
  ...
  async function generateStaticParams() {
    ...
    if (i18n) {
      return source.getPages(i18n.defaultLanguage).map((page: Page) => ({
        slug: page.slugs,
      }));
    }
  ...
  async function generateLocalizedStaticParams() {
    const i18n = source._i18n;
    ...
    if (i18n) {
      return i18n.languages
        ...
        .flatMap((lang: string) =>
          source
            .getPages(lang)
            .filter((page: Page) => page.slugs.length > 0)
            .map((page: Page) => ({
              lang,
              slug: page.slugs,
            })),
