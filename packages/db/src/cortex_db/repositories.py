...
class JobEventRepository:
    ...
    async def get_max_sequence_no(self, job_id: str) -> int:
        """
        Bolt Optimization:
        Calculates the maximum sequence number for a job's events directly in the database.
        Impact: Reduces memory usage and DB transfer time from O(N) (up to 1000 full records) to O(1) (a single integer).
        """
        result = await self._session.execute(
