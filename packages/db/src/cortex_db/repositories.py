...
class JobEventRepository:
    ...
    async def get_max_sequence_no(self, job_id: str) -> int:
        """
        Returns the maximum sequence number for events associated with the given job.
        Optimization: Instead of loading all events and inspecting the last one,
        we use an O(1) SQL MAX() aggregation.
        """
        result = await self._session.execute(
