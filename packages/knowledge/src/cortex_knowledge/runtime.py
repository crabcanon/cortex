...
def build_cognee_runtime(
    ...
    try:
        resolved_config = _resolved_cognee_config(loaded_runtime)
    except ConfigError as exc:
        return DisabledCogneeRuntime(reason=str(exc))
    runtime = PythonCogneeRuntime(config=resolved_config)
...
def _cognee_stable_data_id(
    *,
    ...
    metadata_val = raw_input.get("metadata")
...
def _patch_cognee_tiktoken_unknown_model_fallback() -> None:
    ...
    tokenizer_class.__init__ = _init_with_fallback  # type: ignore[misc]
