"""Evaluation worker bootstrap and execution loop."""
...
from cortex_contracts import EvalJobSubmitRequest
...
from .artifacts import WorkerStorageCaller, persist_evaluation_report
from .hydration import hydrate_evaluation_request


@dataclass(slots=True)
class EvaluationWorkerRunResult:
    status: str
...
class EvaluationWorkerConfig:
    ...
    @classmethod
    def create(cls, worker_id: str | None = None) -> EvaluationWorkerConfig:
        lease_seconds = _int_env("CORTEX_EVALUATION_WORKER_LEASE_SECONDS", 300)
        heartbeat_interval_seconds = _int_env(
            "CORTEX_EVALUATION_WORKER_HEARTBEAT_INTERVAL_SECONDS",
            min(30, max(5, lease_seconds // 3)),
        )
        return cls(
            worker_id=worker_id or new_prefixed_id("eworker"),
...
class EvaluationWorker:
    def __init__(
        ...
        *,
        ...
        job_service: EvaluationJobControlService | None = None,
        storage_service: StorageService | None = None,
        config: EvaluationWorkerConfig | None = None,
    ) -> None:
        self._session_factory = session_factory
        self._evaluation_service = evaluation_service
        self._job_service = job_service or EvaluationJobControlService()
        self._storage_service = storage_service
        self._config = config or EvaluationWorkerConfig.create()

    async def run_once(self) -> EvaluationWorkerRunResult:
        async with CortexUnitOfWork(self._session_factory) as uow:
            ...
            if job is None:
                return EvaluationWorkerRunResult(
                    status="idle", message="No queued evaluation jobs."
                )
            ...
            async with CortexUnitOfWork(self._session_factory) as uow:
                ...
                if current_job is None:
                    return EvaluationWorkerRunResult(
                        status="failed",
                        job_id=job.job_id,
                        message="Claimed evaluation job disappeared before execution.",
                    )
                request = EvalJobSubmitRequest.model_validate(current_job.request_payload)
                request = await hydrate_evaluation_request(
                    uow=uow,
                ...
                try:
                    result = await asyncio.wait_for(
                        self._evaluation_service.run(request),
                        timeout=timeout_seconds,
                    )
                except TimeoutError as exc:
                    if _elapsed_reached_timeout(started_at, timeout_seconds):
                        raise CortexError(
                            code="evaluation_worker_timeout",
                            detail=(
                                "Evaluation job exceeded timeout budget of "
    ...
    async def _record_failure(self, job_id: str, error: Exception) -> EvaluationWorkerRunResult:
        async with CortexUnitOfWork(self._session_factory) as uow:
            latest = await uow.jobs.get(job_id)
            if latest is None:
                return EvaluationWorkerRunResult(status="failed", job_id=job_id, message=str(error))
    ...
    def _execution_timeout_seconds(job: JobRecord) -> int:
        queue_state = job.deployment_context.get("evaluation_worker")
...
def build_worker(settings: CortexSettings | None = None) -> EvaluationWorkerRuntime:
    ...
    worker = EvaluationWorker(
        ...
        evaluation_service=build_evaluation_service(loaded_settings.evaluation, runtime_config),
        storage_service=StorageService(loaded_settings.s3),
        config=EvaluationWorkerConfig.create(),
    )
    return EvaluationWorkerRuntime(worker=worker, engine=engine)


def bootstrap_message() -> str:
    return "cortex evaluation worker bootstrap ready"
