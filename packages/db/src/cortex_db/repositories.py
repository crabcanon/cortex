...
class JobEventRepository:
    ...
    async def get_max_sequence_no(self, job_id: str) -> int:
        # Optimized sequence calculation: Uses an O(1) SQL aggregation query to determine
        # the next sequence number without loading unneeded ORM objects into memory.
        stmt = select(func.coalesce(func.max(JobEventModel.sequence_no), 0)).where(
