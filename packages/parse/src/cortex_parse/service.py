...
class ParsePersistenceService:
    ...
    async def persist(
        ...
        *,
        ...
        attempts_to_add: list[ParseRunAttemptRecord] = []
    ...
    async def _persist_document_graph(
        ...
        *,
        ...
        tags_to_add: list[DocumentTagRecord] = []
        for tag in list(dict.fromkeys([*document.category_tags, *document.labels])):
            tags_to_add.append(DocumentTagRecord(document_id=created.document_id, tag=tag))
        if tags_to_add:
            await uow.document_tags.add_many(tags_to_add)

        chunks_to_add: list[DocumentChunkRecord] = []
        ...
        if request.persistence.persist_artifacts and result.artifacts is not None:
            artifacts_to_add: list[DocumentArtifactRecord] = []
