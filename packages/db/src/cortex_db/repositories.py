...
class JobEventRepository:
    ...
    async def get_max_sequence_no(self, job_id: str) -> int:
        # Bolt performance optimization: Using SQL MAX function avoids loading thousands
        # of events into memory just to determine the next sequence number. O(1) instead of O(N).
        result = await self._session.execute(
