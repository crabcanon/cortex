...
class ParsePersistenceService:
    ...
    async def persist(
        ...
        *,
        ...
        if attempt_records:
            uow.parse_run_attempts.add_many(attempt_records)
    ...
    async def _persist_document_graph(
        ...
        *,
        ...
        for tag in list(dict.fromkeys([*document.category_tags, *document.labels])):
            tag_records.append(DocumentTagRecord(document_id=created.document_id, tag=tag))
        if tag_records:
            uow.document_tags.add_many(tag_records)
        ...
        if chunk_records:
            uow.document_chunks.add_many(chunk_records)
        ...
        if request.persistence.persist_artifacts and result.artifacts is not None:
            ...
            if artifact_records:
                uow.document_artifacts.add_many(artifact_records)
